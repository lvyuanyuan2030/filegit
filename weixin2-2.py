...
#通过搜狗搜索中的微信搜索入口来爬取公共号文章：存储所有文章信息到MySQL数据库中的weixin数据库，以公众号名称创建表格；存储文章正文内容到Mongodb数据库
...
class weixin_spider:
	...
	def switch_arctiles_to_diclist(self, articles):
		...
		if articles:
			for article in articles.items():
				...
				text_list.append(self.parse_one_article_text(article))
				i += 1
		return articles_list,text_list
	...
	#解析单篇文章文本内容
	def parse_one_article_text(self, article):
		text_dict = {}
		list2 = []
		#获取标题
		title = article('h4[class="weui_media_title"]').text()
		...
		#存储文章文本内容到本地
		contentfiletitle=self.keywords+'/'+title+'.txt'
	...
	def mongo_insertdata(self, collectionname,diclist): 
		# 连接数据库
		client = pymongo.MongoClient(host='localhost', port=27017)
		#指定数据库
		db = client.weixin
		#指定集合 有两种方式，db.xx和db[xx]，为了能根据变量名称动态创建集合，用db[]方式才行
		strg = '%s' %(collectionname)
		collection = db[strg]
		#插入数据
		result = collection.insert(diclist)
	...
	def run(self):
		...
		if self.need_verify(selenium_html):
			...
			# Step 6: 把微信文章数据封装成2个字典list
			self.log(u'开始整合微信文章数据为字典')
			articles_list,text_list = self.switch_arctiles_to_diclist(articles)
			
			# Step 7: 把所有文章信息字典list存入MySQL数据库
			self.sqldb_insertdata(self.keywords, articles_list)
			
			# Step 8: 把文章正文字典list存入mongodb数据库
			self.mongo_insertdata(self.keywords, text_list)
