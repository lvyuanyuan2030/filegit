...
#代码可执行:存储所有历史文章列表为csv格式，存储所有历史文章正文为txt格式
...
class spider_wechat:
        ...
        while 1:
            ...
            if tags1:
                if(tags1.get('style')) == 'display: none;':
                    ...
                    driver.execute_script(js)#模拟滚动条下滑操作
                    top += 1
                    time.sleep(3) 
                else:
                    break
    ...
    def parse_one_article(self, article,i,writer):
        ...
        self.log('标题： %s' % title)
        #获取标题对应的地址
        url = article.select('h4[class="weui_media_title"]')[0].get('hrefs')
        self.log('地址： %s' % url)
        #获取概要内容
...
if __name__ == '__main__':
    print '''
            ...
            *****************************************
    '''
    wechat_name = raw_input(u'输入要爬取的公众号ID')
